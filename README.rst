...
After building your documentation with the Notion builder, you can upload it to Notion using the included command-line tools.
The upload process requires two steps to handle both file uploads and content creation.
...
3. Create a SHA mapping file (initially empty): ``{"sha256_hash": {"file_id": "notion_file_id", "file_name": "filename.ext"}}``

Two-Step Upload Process
~~~~~~~~~~~~~~~~~~~~~~~~

The upload process consists of two CLI tools that must be run in sequence:

Step 1: Upload Files
^^^^^^^^^^^^^^^^^^^^

First, use the ``upload-files`` command to upload any files referenced in your documentation and update the SHA mapping:

.. code-block:: console

   $ upload-files --mapping-file ./notion-sha-mapping.json --build-file ./build/index.json

Arguments:

- ``--mapping-file``: Path to the SHA mapping JSON file that tracks uploaded files
- ``--build-file``: Path to the JSON file generated by the Notion builder
...
- Update the SHA mapping file with file IDs and metadata
- Remove unused files from the mapping

Step 2: Upload Documentation Content
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Next, use the ``notion-upload`` command to create or update the Notion page with your documentation content:
...
- ``--sha-mapping``: Path to the SHA mapping JSON file (required)
...
.. code-block:: console
   ...
   # Step 1: Upload files and update SHA mapping
   $ upload-files --mapping-file ./notion-sha-mapping.json --build-file ./build/index.json
